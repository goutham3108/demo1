...
slow (TCG software emulation, 5×–20× slower) — see
[**Troubleshooting → G. VMs feel extremely slow / how to enable KVM**](#g-vms-feel-extremely-slow--setuppy-shows-warning-kvm-not-enabled-slower-vm)
for the full diagnose-and-fix flow.
...
min without it, and per-action latency (slider drags, range
recompute) suffers proportionally.

Quick check:

```bash
ls -l /dev/kvm 2>&1                 # must exist
groups | tr ' ' '\n' | grep '^kvm$' # must print: kvm
```

If either check fails, follow the full diagnose-and-fix flow in
[**Step 3 — Confirm KVM is usable (and enable it if not)**](#step-3--confirm-kvm-is-usable-and-enable-it-if-not).
That section covers the four real cases: missing `kvm` group
membership, BIOS/UEFI virtualization disabled, KVM kernel module not
loaded, and WSL2 nested virtualization not exposed by the Windows host.

After applying the fix, destroy the half-provisioned VMs (their qcow2
disks may already have partial cloud-init writes from the slow run)
and rerun `setup.py`:
